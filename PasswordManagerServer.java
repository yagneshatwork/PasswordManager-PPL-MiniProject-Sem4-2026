...
public class PasswordManagerServer {
    ...
    private void saveVault() {
        try {
            ...
            // --- ENCRYPTION FUNCTION & ALGORITHM START ---
            // Algorithm Customization: AES-GCM (Advanced Encryption Standard in Galois/Counter Mode)
            // GCM mode provides both confidentiality and data authenticity (integrity) simultaneously.
            Cipher cipher = Cipher.getInstance(ALGORITHM);
            cipher.init(Cipher.ENCRYPT_MODE, secretKey, new GCMParameterSpec(GCM_TAG_BITS, iv));
            byte[] ciphertext = cipher.doFinal(plaintext);
            // --- ENCRYPTION FUNCTION & ALGORITHM END ---
    ...
    private void loadVault(String masterPassword) throws Exception {
        ...
        // --- DECRYPTION FUNCTION & ALGORITHM START ---
        // Algorithm Customization: AES-GCM (Advanced Encryption Standard in Galois/Counter Mode)
        // Decrypts the data while verifying its authentication tag to ensure the file hasn't been improperly modified.
        Cipher cipher = Cipher.getInstance(ALGORITHM);
        cipher.init(Cipher.DECRYPT_MODE, secretKey, new GCMParameterSpec(GCM_TAG_BITS, iv));
        byte[] plaintext = cipher.doFinal(ct);
        // --- DECRYPTION FUNCTION & ALGORITHM END ---
    ...
    private SecretKey deriveKey(String password, byte[] salt) {
        try {
            // --- KEY DERIVATION ALGORITHM START ---
            // Algorithm Customization: PBKDF2 (Password-Based Key Derivation Function 2) with HMAC-SHA-256.
            // Adds computational work to make brute-forcing the master password statistically infeasible.
            SecretKeyFactory skf = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA256");
            KeySpec spec = new PBEKeySpec(password.toCharArray(), salt, ITER, KEY_BITS);
            byte[] keyBytes = skf.generateSecret(spec).getEncoded();
            // --- KEY DERIVATION ALGORITHM END ---
